...
public interface GoodMapper {
    ...
    @Select("select * from good where price>0 and number>0")
    List<Good> getall();
    ...
    @Select("select `number` from `good` where `name` = #{name};")
    int getgoodnum(String name);
    ...
    @Update("update good set number=#{arg0} where name=#{arg1};")
    void number(int number,String name);
    ...
    @Select("select `price` from `good` where 'id' = #{id};")
    float selectPrice(int id);
