<template>
  <div class="main">
    ...
    <div>
      <el-button type="primary" class="button7(scope.row.custid,scope.row.goodid)" @click="gotoshoucang">{{ shoucangjia }}</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleClick(custid, goodid) {
      this.FormData.goodname = goodid
      this.FormData.custname = custid
      console.log(this.FormData)
      if (this.shoucangjia === '加入购物车') {
        request.post('/api/hisg/***', this.FormData).then(res => {},
          this.$confirm('操作成功', '提示', {
            confirmButtonText: '确定',
            type: 'warning',
            center: true
          },
          this.statename = '移出购物车'
          ))
      }
      if (this.shoucangjia === '移出购物车') {
        request.post('/api/hisg/***', this.FormData).then(res => {},
          this.$confirm('操作成功', '提示', {
            confirmButtonText: '确定',
            type: 'warning',
